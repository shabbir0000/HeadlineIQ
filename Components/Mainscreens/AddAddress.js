...
const AddAddress = ({ navigation, route }) => {
  const { pid, rfname, rlname, phonee, area_l, area_v, fuel_area } =
    route.params;
  const [email, setEmail] = useState();
  const [fname, setfname] = useState(rfname);
  const [lname, setlname] = useState(rlname);
  const [password, setPassword] = useState('');
  const [fuel, setfuel] = useState(fuel_area);
  const [phone, setphone] = useState(phonee);
  const [address, setaddress] = useState();
  const [Getdata, setGetdata] = useState([]);
  const [value3, setValue3] = useState(area_v);
  const [label3, setlabel3] = useState(area_l);
  ...
  return (
    <View style={[{ backgroundColor: '#FEF0F0' }, tw`flex-1`]}>
      <Screensheader
        name={pid ? 'UPDATE ADDRESS' : 'ADD ADDRESS'}
        left={5}
        onPress={() => navigation.goBack()}
      />
        ...
        <ScrollView>
          ...
          >
            <Text style={tw`text-2xl underline mt-5 font-bold`}>
              {pid ? 'UPDATE DELIVERY ADDRESS' : 'ADD DELIVERY ADDRESS'}
            </Text>
          ...
          <View>
            ...
            >
              <Text style={tw`text-gray-400`}>Address Title</Text>
              <Input
                value={fname}
            ...
            >
              <Text style={tw`text-gray-400`}>Full Address</Text>
              <Input
                value={lname}
