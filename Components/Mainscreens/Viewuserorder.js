...
const Viewuserorder = ({ navigation, route }) => {
  const { vendoremail, myorderid, timeAgo } = route.params;
  ...
  useEffect(() => {
    AsyncStorage.getItem('userid').then(id => {
      ...
      const q = query(coll, where('email', '==', vendoremail));
          ...
          >
            ...
            <View>
              <Text style={tw`mr-1 font-bold text-base`}>
                {GetData[0]?.selecteduser?.discount > 0
                  ? parseInt(parseInt(finalAmount) + GetData[0]?.selecteduser?.vendordc) -
                    parseFloat(GetData[0]?.selecteduser?.discount)
