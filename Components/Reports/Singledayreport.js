...
const Singledayreport = () => {
  ...
  const onChange = async (event, date) => {
    ...
    // getCompletedOrdersSummaryByDate(formattedDate).
    const todaySummary = await getCompletedOrdersSummaryByDate(formattedDate);
    console.log('Aaj ke completed orders ka summary:', todaySummary);
    settodaysummery(todaySummary);
    // generateSalesReport(formattedDate);
    // fetchstatusDatabydate(formattedDate);
  };

  const onChange1 = async (event, date) => {
  ...
  const onChange2 = async (event, date) => {
  ...
  const fetchCompletedSummary = async () => {
    // Default — aaj ki date
    const todaySummary = await getCompletedOrdersSummaryByDate();
    console.log('Aaj ke completed orders ka summary:', todaySummary);
    settodaysummery(todaySummary);
    // Custom date example
    // const customSummary = await getCompletedOrdersSummaryByDate(
    //   '10:15:2025',
    // );
    // console.log('18 Oct ke completed orders ka summary:', customSummary);
    // settodaysummery(customSummary);
  };
  ...
  const getCompletedOrdersSummaryByDate = async (customDate = null) => {
    try {
      // 🔹 Step 1: Current date ya custom date set karo
      let formattedDate;
      if (customDate) {
        formattedDate = customDate; // e.g. "10:20:2025"
      } else {
        const today = new Date();
      ...
      // 🔹 Step 2: Firestore query banao (sirf completed orders aur given date)
      ...
      // 🔹 Step 3: Totals initialize
      let totalVendorDC = 0;
      let totalSubtotal = 0;
      let totalBill = 0;

      // 🔹 Step 4: Sum all fields
      snapshot.forEach(doc => {
        const data = doc.data();
        totalVendorDC += Number(data.vendordc || 0);
        totalSubtotal += Number(data.subtotal || 0);
  ...
  const getCompletedOrdersSummaryByRange = async (
    fromDate = null,
    toDate = null,
  ) => {
    try {
      ...
      // 🔹 Step 1: Default dates (agar user na de)
      const today = new Date();
      const month = (today.getMonth() + 1).toString().padStart(2, '0');
      const day = today.getDate().toString().padStart(2, '0');
      const year = today.getFullYear();
      ...
      // 🔹 Step 2: Dates set karo
      const startDate = fromDate || todayFormatted;
      const endDate = toDate || todayFormatted;
      console.log('setp 2 ', startDate, '::::::', endDate);

      // 🔹 Step 3: Firestore query banao
      const coll = collection(db, 'Orders');
      const q = query(
        coll,
        where('orderstatus', '==', 'completed'),
      ...
      const snapshot = await getDocs(q);
      console.log('setp 3 ');
      // 🔹 Step 4: Totals initialize
      let totalVendorDC = 0;
      let totalSubtotal = 0;
      let totalBill = 0;

      // 🔹 Step 5: Sum all documents
      snapshot.forEach(doc => {
        const data = doc.data();
        totalVendorDC += Number(data.vendordc || 0);
        totalSubtotal += Number(data.subtotal || 0);
      ...
      console.error(
        '🔥 Error fetching completed order summary by range:',
        error,
      );
      return {
        fromDate,
  ...
  const getCompletedOrdersSummaryByRangebytime = async (
    fromDate = null,
    toDate = null,
  ) => {
    try {
      ...
      const now = new Date();
      const mystart = fromDate ? new Date(fromDate) : now;
      const myend = toDate ? new Date(toDate) : now;
      ...
      const q = query(
        coll,
        where('orderstatus', '==', 'completed'),
        where('timestamp', '>=', mystart),
        where('timestamp', '<=', myend),
      );
      ...
      <ScrollView style={tw`bg-white`}>
        ...
        >
          <View
            style={[
              { width: width * 0.92 },
              tw`  h-100 mb-5 mr-2 ml-2 mt-5 self-center justify-evenly border border-gray-200 rounded-xl bg-white shadow-xl `,
            ]}
          >
            <View style={tw`flex-row justify-between`}>
              ...
              >
                <View>
                  ...
                  <Text
                    numberOfLines={1}
                    style={tw`ml-6 font-semibold text-green-500 text-base`}
                  >
                    {todaysummery?.totalBill}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={tw`flex-row justify-between`}>
              <View
                style={[{ width: width * 0.9 }, tw` flex-row items-center`]}
              >
                <View>
                  <Text
                    numberOfLines={1}
                  ...
                  >
                    {' '}
                    {'Total Earning Without Delivery Charges'}
                  </Text>
                  <Text
                    numberOfLines={1}
                    style={tw`ml-6 font-semibold text-green-500 text-base`}
                  >
                    {todaysummery?.totalSubtotal}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={tw`flex-row justify-between`}>
              ...
              >
                <View>
                  <Text
                    numberOfLines={2}
                    style={tw`ml-5 font-semibold text-center text-base`}
                  >
                    {' '}
                    {'Total Earning Without Delivery Charges And Tax'}
                  </Text>
                  <Text
                    numberOfLines={1}
                    style={tw`ml-6 font-semibold text-green-500 text-base`}
            ...
            <View style={tw`flex-row justify-between`}>
              ...
              >
                <View>
                  ...
                  >
                    {' '}
                    {'Total Delivery Charges Earning'}
                  </Text>
                  <Text
                    numberOfLines={1}
                    style={tw`ml-6 font-semibold text-green-500 text-base`}
                  >
                    {todaysummery?.totalVendorDC}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={tw`flex-row justify-between`}>
              ...
              >
                <View>
                  ...
                  >
                    {' '}
                    {'Total Earn Without Any Tax'}
                  </Text>
                  <Text
                    numberOfLines={1}
                    style={tw`ml-6 font-semibold text-green-500 text-base`}
                  >
                    {todaysummery?.totalSubtotal + todaysummery?.totalVendorDC}
                  </Text>
                </View>
              </View>
            </View>
              ...
              <View style={tw` flex-col  h-60 justify-around`}>
                ...
                <View style={tw`mt-5`}>
                  <TouchableOpacity
                    onPress={() => {
                      const all = async () => {
                        const data = await getCompletedOrdersSummaryByRange(
                          datename1,
                          datename2,
                        );
                        settodaysummery(data);
                      };
                      all();
                    }}
                  >
                ...
                <View style={tw`mt-5`}>
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      const all = async () => {
                        const data =
                          await getCompletedOrdersSummaryByRangebytime(
                            datetime1,
                            datetime2,
                          );
                        settodaysummery(data);
