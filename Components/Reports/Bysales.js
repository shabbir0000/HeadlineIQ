...
const Bysales = () => {
  ...
  const generateSalesReportfromtodate = async (
    ...
    try {
      ...
      // ✅ Step 5: Calculate total sale for each product (only completed orders)
      ...
      // ✅ Step 6: Add zero for products that didn’t sell
      ...
      // ✅ Step 8: Sort descending (highest sale first)
      ...
      toggleModal();
      console.log('✅ Final Report (with completed orders only):', report);
      return report;
    } catch (error) {
  ...
  const generateSalesReportfromtodateandtime = async (
    ...
    try {
      ...
      // const start = parseDate(mystart);
      // const end = parseDate(myend);
      ...
      setProductdata(report);
      toggleModal1();
      console.log('✅ Final Report (with time):', report);
      return report;
    } catch (error) {
      console.error('❌ Error generating report:', error);
    }
  ...
  return (
    <View style={tw`bg-white`}>
      <ScrollView>
        <View
          style={tw` flex-row w-80 h-14 mt-3 justify-between self-center items-center`}
        >
