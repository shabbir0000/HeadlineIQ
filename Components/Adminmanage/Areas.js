...
const EvenOddColumns = ({ data, navigation }) => {
  ...
  const addcat = async () => {
    ...
    if (!comapny) {
      ...
      AsyncStorage.getItem('email').then(email => {
        setDoc(doc(db, 'Sub_Areas', userid), {
  ...
  const updateCat = async () => {
    if (!comapny || !catid) {
      ...
      try {
        ...
        await updateDoc(doc(db, 'Sub_Areas', catid), {
          company: comapny.trim().toLowerCase(),
          fuel_charges: fuel,
          timestamp: serverTimestamp(),
          //   catimg: filterimages,
  ...
  const deleteCat = async docId => {
    try {
      ...
      await deleteDoc(doc(db, 'Sub_Areas', docId));
        ...
        <View style={tw`h-55 flex-row justify-around mt-5`}>
          ...
          >
            {userflag ? (
              ...
              <View style={tw`flex-row w-35 mt-2 justify-between`}>
                <TouchableOpacity
                  onPress={() => {
                    toggleModal(), setmname('UPDATE AREAS');
            ...
            >
              FUEL {item.even.selecteduser.fuel_charges}/=RS
            </Text>
          </View>
          ...
          {item.odd && (
            ...
            >
              ...
              >
                FUEL {item.odd.selecteduser.fuel_charges}/=RS
              </Text>
            </View>
          )}
        </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {userflag ? (
        ...
        <FAB
          onPress={() => {
            toggleModal(), setfuel(''), setmname('ADD AREAS');
          }}
            ...
            <View style={tw` flex-col  h-60 justify-around`}>
              ...
              {loading ? (
                ...
                <View style={tw`mt-5`}>
                  <TouchableOpacity
                    onPress={() => {
                      mname === 'ADD AREAS' ? addcat() : updateCat();
                    }}
                  >
                    <View
                      style={[
