...
const Manageriders = ({ navigation }) => {
  ...
  const getriderbystatus = async status => {
    let unSubscribe;
    AsyncStorage.getItem('email').then(email => {
      const coll = collection(db, 'subs_users');
      const q = query(
  ...
  return (
    <View style={[tw`flex-1 bg-white`]}>
      <View
        style={[
          { width: width * 0.9 },
          tw`h-14 mt-3 self-center flex-row items-center justify-around`,
        ]}
      >
        <Dropdown
          ...
          onChange={item => {
            ...
            if (item.label === 'Active') {
              getriderbystatus(true);
            } else {
              getriderbystatus(false);
            }
...
const EvenOddColumns = ({ data, navigation, uuid }) => {
  ...
  const Riderstatus = async (pid, active) => {
    AsyncStorage.getItem('Guest').then(guest => {
      if (guest == 'No') {
        const bookdd = async () => {
          setloading1(true);
          try {
            updateDoc(doc(db, 'subs_users', pid), {
              active: !active,
            })
              .then(() => {
                console.log('done');
            ...
            >
              <View style={tw`mb-3 w-75 `}>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      navigation.navigate('Addrider', {
                        ...
                        rfname: item.selecteduser.fname,
                        rlname: item.selecteduser.lname,
                        phonee: item.selecteduser.phoneNumber,
                        addresss: item.selecteduser.address,
                        emaill: item.selecteduser.email,
                      });
                    }}
                  >
                    <Image
                ...
                >
                  ...
                  {'\n'}
                  {item?.selecteduser.fname.toUpperCase()}
                </Text>
                ...
                >
                  ...
                  {'\n'}
                  {item?.selecteduser.lname.toUpperCase()}
                </Text>
                ...
                >
                  ...
                  {'\n'}
                  {item?.selecteduser.address.toUpperCase()}
                </Text>
